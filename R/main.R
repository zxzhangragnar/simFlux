...
find_anchor_rid <- function(net) {
  ## anchor edge
  nozero_count = apply(net, 2, function(c)sum(c!=0))
...
## cycle
get_anchor_cycle_affiliated_edges <- function(hsa_net, g, anchor_rid, anchor_node) {
    ...
    for (i in 1:length(cycles_cname_directed)) {
      ...
      if(length(intersect(E(g1)$rid, anchor_rid)) > 0) {
        for (cn in cycles_cname_directed[[i]]) {
          ...
          for (ind in in_nodes) {
            in_suc = bfs(g, root=ind, "in", unreachable = FALSE,
          ...
          for (od in out_nodes) {
            out_suc = bfs(g, root=out_nodes[1], "out", unreachable = FALSE,
                          order=TRUE, rank=TRUE, father=TRUE, pred=TRUE,
                          succ=TRUE, dist=TRUE)
...
#########################################################################################
plot_flux_sim <- function(solutions, solutions_error, g, graph_name) {
  ...
  png(plot_name, height=png_size, width=png_size, units="in", res=200)
  ...
  par(mfrow=c(1, 2))
  plot(g, edge.label = paste0(E(g)$rid,":",E(g)$value),
  ...
  mtext("origin", side=1)
  plot(ge, edge.label = paste0(E(ge)$rid,":",E(ge)$value),
...
write_flux_sim <- function(solutions, solutions_error, csv_name) {
  ...
  write.table(X, paste0(csv_name, "_sim.csv"),row.names=TRUE,col.names=FALSE,sep=",")
}
...
## main
library(readr)
flux_sim_main <- function(accurate_1, solution_num, metabolism_network_path) {
  ...
  for (i in 1:times) {
    ...
    bvls_solution = get_bvls_solution(upstream_value, boundary_value, cmMat, anchor_cycle_affiliated_edges, FALSE)
...
get_gene_df <- function(times, accurate, max_mgene_len, system_rightside, mod_genes_name, mod_genes_table) {
  ...
  if (ncol(gene_df) > 0) {
    sample_nums = str_pad(1:ncol(gene_df),floor(log10(ncol(gene_df))) + 1,side="left","0")
...
############################### generate #####################################
gene_expression_sim_main <- function(module_gene_json_path, module_sim_flux_path, times, accurate, csv_rowno_in_python=0) {
  ...
  gene_df = get_gene_df(times, accurate, max_mgene_len, system_rightside_multi_mean, mod_genes_name, mod_genes_table)
  # if (ncol(gene_df) > 0) {
...
#'
#'
sim_metabolism_flux <- function(precision=0.00000001, times=500, metabolism_network_path) {
  # setwd("E:/scPlus_universal/simulate/flux_generater/")
...
#'
#'
sim_metabolism_gene_expression <- function(module_gene_json_path, module_sim_flux_path, times=900, precision=0.00000000000001) {
  library(readr)
  library(bvls)
  library("rjson")
  library("stringr")
  ...
  gene_df_sim = gene_expression_sim_main(module_gene_json_path, module_sim_flux_path, times, precision, 0)
